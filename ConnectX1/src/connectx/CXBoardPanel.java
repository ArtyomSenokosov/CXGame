...
public class CXBoardPanel extends JPanel implements MouseListener, MouseMotionListener, Serializable {
	...
	protected CXGame.CXPlayerType[] Player;
	protected CXPlayer[] ComPlayer;

	protected CXGame.CXGameType gameType;
	...
	public CXBoardPanel(CXBoard board, CXGame.CXGameType type, Border bord, int cell_size, CXPlayer[] ComPlayer,
						CXGame.CXPlayerType[] Player, JLabel statusBar) {
	...
	public void paintComponent(Graphics g) {
		...
		case OPEN:
			...
			String msg = Player[board.currentPlayer()] == CXGame.CXPlayerType.COMPUTER ? "Click to run"
					: "Click on white bar to select column";
			String name = Player[board.currentPlayer()] == CXGame.CXPlayerType.COMPUTER
		...
		case DRAW:
			...
			break;
		case WINP1:
			String name1 = Player[0] == CXGame.CXPlayerType.COMPUTER ? ComPlayer[0].playerName() : "Human";
			statusBar.setForeground(Color.RED);
			statusBar.setText("Yellow (" + name1 + ") Won! Click reset to play again.");
			break;
		case WINP2:
			String name2 = Player[1] == CXGame.CXPlayerType.COMPUTER ? ComPlayer[1].playerName() : "Human";
			statusBar.setForeground(Color.RED);
	...
	public void drawMousePointer(Graphics2D g2) {
		...
		if (!animatingCoinDrop && Player[board.currentPlayer()] == CXGame.CXPlayerType.HUMAN) {
	...
	public void mouseMoved(MouseEvent mm) {
		if (Player[board.currentPlayer()] == CXGame.CXPlayerType.HUMAN) {
	...
	public void mouseClicked(MouseEvent m1) {
		if (gameState == CXGameState.OPEN) {
			if (Player[board.currentPlayer()] == CXGame.CXPlayerType.HUMAN) { // Human player
